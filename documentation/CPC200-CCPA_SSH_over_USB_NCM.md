...
> Verified live on 2026‑05‑30: firmware build dated `2025.09.26` (a build/capture datestamp, not a release number — this unit runs the `2025.10` firmware release line documented elsewhere in these notes), kernel `3.14.52+g94d07bb` (armv7l), hostname `sk_mainboard`. Hardware: A15W revision (IW416 Wi‑Fi; other CCPA hardware revisions ship different WLAN silicon, e.g. the Realtek RTL88x2CS). Host: macOS (Apple Silicon). The same approach works on Linux; Windows needs a CDC‑NCM driver (see notes).
