...
| `CallQuality` | `CallQuality` | 0-2 | 1 | **BUGGY** - Does NOT set VoiceQuality (see Known Bugs below) |
...
| `NotCarPlayH264DecreaseMode` | `NotCarPlayH264DecreaseMode` | 0-2 | 0 | **DEAD KEY** — zero runtime xrefs (binary-verified). Intended to block adaptive bitrate, but never read at runtime in fw 2025.10.15. |

**SpsPpsMode Values:**
| Value | Mode | Description |
|-------|------|-------------|
| 0 | Passthrough | Forward SPS/PPS as-is from phone, no modification (does NOT read LastPhoneSpsPps — itself a dead key with zero runtime xrefs) |
...
```c
// BitRate sent in CarPlay negotiation; firmware applies a 0.25 throttle
// factor (value × 0.25 = effective Mbps hint), e.g. 16-20 -> ~4-5 Mbps
carplay_set_preferred_bitrate(VideoBitRate * 0.25 * 1000000);
...
| `USBConnectedMode` | `USBConnectedMode` | 0-2 | 0 | USB gadget function selection (consumed by start_mtp.sh, writes /sys/class/android_usb_accessory/android0/functions) |
...
| Value | Mode | Description |
|-------|------|-------------|
| 0 | mtp,adb | Both MTP and ADB gadget functions |
| 1 | mtp only | MTP only (no ADB) |
| 2 | adb only | ADB only (no MTP) |
...
| Web API Name | riddleBoxCfg Name | Range | Default | Firmware Hook |
|--------------|-------------------|-------|---------|---------------|
| `MouseMode` | `MouseMode` | 0-1 | 1 | Convert touch to cursor (default 1 per riddleBoxCfg --info) |
...
| `lightType` | `lightType` | 1-3 | 3 | **DEAD KEY** — zero runtime xrefs, not in BoxSettings table (binary-verified). LED behavior is driven by the separate rgb*/ledTimingMode parameters, not lightType. |
...
| Web API Name | riddleBoxCfg Name | Range | Default | Firmware Hook |
|--------------|-------------------|-------|---------|---------------|
| `autoPlay` | `AutoPlauMusic` | 0-1 | 0 | Auto-play music on CarPlay connect. ⚠️ Settable via web UI only — BoxSettings JSON `autoPlay` is NOT mapped in ARMadb-driver (key absent from the ARMadb-driver string table, binary-confirmed). |
...
| `AndroidWorkMode` | `AndroidWorkMode` | 0-5 | 1 | Android connection mode (0=Idle, 1=AA, 2=CarLife, 3=Mirror, 4=HiCar, 5=ICCOA). Persisted/config default is 1, but runtime state resets to 0 on phone disconnect / power-cycle — the host must set it back to 1 each session for Android Auto support. |
...
| Web API Name | riddleBoxCfg Name | Range | Default | Firmware Hook |
|--------------|-------------------|-------|---------|---------------|
| `MediaPacketLen` | `MediaPacketLen` | 200-20000 | 200 | Media USB bulk size (range 200-20000 per `riddleBoxCfg --info`, the definitive source; Tts/VrPacketLen use 200-40000). |
...
| Improved Fluency | `ImprovedFluency` | 0=Off, 1=On | **DEAD KEY** — no runtime effect (see firmware hook column above; canonical entry in `configuration.md`) |
...
| ~~`AutoPlauMusic`~~ | Auto Play | **NOT overridable** — `autoPlay` is NOT mapped via BoxSettings (key absent from the ARMadb-driver string table, binary-confirmed); settable via web UI only. Previously listed here as overridable in error. |
