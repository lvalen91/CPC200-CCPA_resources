...
> **Note:** `accZLP` is **kernel/gadget-controlled** (logged by the `f_accessory` driver's `acc_write`), not config-controlled. In the adapter tty logs `accZLP` is **0** regardless of transfer size, and `USBTransMode` appears nowhere. As documented in `usb_zlp_bug.md`, the ZLP behavior is **kernel-module-level only**: the `USBTransMode`→`start_aoa.sh`→`accZLP` config path described above is **inert** and has no observed runtime effect. The userspace `_SendDataToCar … may need send ZLP` message is advisory; the kernel makes the actual ZLP decision.
...
**Type:** Number | **Default:** 200 | **Range (per `riddleBoxCfg --info`):** MediaPacketLen 200-20000; TtsPacketLen/VrPacketLen 200-40000
...
> **Android Auto note (binary-confirmed Jun 2026):** `HU_SAFEAREA_INFO` (main-screen SafeArea) is **CarPlay-only** — only `AppleCarPlay._CopyDisplayDescriptions` reads it; neither `ARMadb-driver` nor `ARMAndroidAuto` does. `ARMAndroidAuto`'s aasdk **openauto-v2** `VideoConfig` proto has only `margin_width`/`margin_height` (no `ui_config`/`content_insets`), so AA gets a **symmetric letterbox margin only** — sized from the `androidAutoSizeW/H` keys, **not** from `HU_VIEWAREA_INFO` (whose only code xref is `ProceessCmdOpen`, the CarPlay/Open handler). There is **no main-screen AA content_insets/SafeArea via the dongle** — it's structurally impossible. The cluster `HU_NAVISCREEN_*` pair is a separate shared path. See **[Android Auto: margins only — no main-screen SafeArea](#android-auto-margins-only--no-main-screen-safearea-binary-verified-jun-2026)** below.
...
#### Android Auto: margins only — no main-screen SafeArea (Binary-Verified Jun 2026)

The `_CopyDisplayDescriptions` trace above is the **CarPlay** consumer, and it is the **only** consumer
of `HU_SAFEAREA_INFO`. r2 analysis of the unpacked firmware binaries (2025.10) is decisive:

- **`ARMAndroidAuto`** speaks the **aasdk openauto-v2** Android Auto Projection SDK (build path
  `/home/hcw/M6PackTools/HeweiPackTools/AndroidAuto_Wireless/openauto-v2/../AndroidAutoSdk-v2/aasdk_proto/`).
  Its embedded `VideoConfigData.proto` descriptor defines `aasdk.proto.data.VideoConfig` with exactly
  these fields: **`video_resolution`, `video_fps`, `margin_width`, `margin_height`, `density`,
  `additional_depth`, `real_density`** (the AV channel wraps them in `video_configs`). There is **no
  `ui_config`, no `content_insets`, no `margins` nested message, and no per-edge `Insets` type anywhere
  in the binary.** This SDK predates AAP's `ui_config` — so `ARMAndroidAuto` is *structurally incapable*
  of expressing a content-inset SafeArea; it can only emit a symmetric letterbox (`margin = tier − config`).
- **AA's view rectangle comes from the `androidAutoSizeW`/`androidAutoSizeH` BoxSettings keys**
  (→ `AndroidAutoWidth`/`AndroidAutoHeight` in `riddle.conf`, read by `ARMAndroidAuto` via `GetBoxConfig`),
  **not** from `HU_VIEWAREA_INFO`. In `ARMadb-driver`, `HU_VIEWAREA_INFO`'s only code xref is
  `fcn.00021cb0` (`ProceessCmdOpen`, the Open-message handler that auto-rewrites it for the CarPlay /
  AirPlay path); it is **never** referenced on the AA path. The bare main-screen `HU_SAFEAREA_INFO` is
  not referenced by `ARMadb-driver` at all (CarPlay-only, via `AppleCarPlay`).

**Consequence:**

| Path | View Area | Main-screen Safe Area |
|---|---|---|
| CarPlay (`AppleCarPlay`) | `HU_VIEWAREA_INFO` → AirPlay viewAreas | `HU_SAFEAREA_INFO` → AirPlay safeArea (full support) |
| Android Auto (`ARMAndroidAuto`, via dongle) | symmetric letterbox margins from `androidAutoSizeW/H` (`margin = tier − config`); **not** from any `HU_*AREA` file | **none** — proto has no `content_insets`/`ui_config` field; structurally impossible |
| Cluster / NaviScreen | `HU_NAVISCREEN_VIEWAREA_INFO` | `HU_NAVISCREEN_SAFEAREA_INFO` (+ `naviScreenInfo.safearea`) — shared path |

So the **main Android Auto screen does NOT get a CarPlay-style content-inset SafeArea through the
Carlinkit dongle** — and neither `HU_VIEWAREA_INFO` nor `HU_SAFEAREA_INFO` feeds the AA main screen at
all. A host that needs genuine AA `ui_config.content_insets` (e.g. LIVI's independent AAP/AoA stack)
must speak the Android Auto Projection protocol *directly* — not rely on the dongle. The AAP rule
"SafeArea ⊆ ViewArea or the phone rejects the config" is a validation in that direct stack
(`content_insets ⊆ margins`), **not** a dongle behavior.

**Provenance (binary-confirmed):** `VideoConfigData.proto` descriptor + string/xref analysis of
`ARMAndroidAuto_reconstructed` and `ARMadb-driver_running_unpacked` via radare2 (firmware 2025.10,
`_evidence/cpc200_ccpa_firmware_binaries/analysis/`). The `VideoConfig` field set is read directly from the
embedded protobuf descriptor (stronger than disassembly — it's the schema). This **corrects an earlier
same-day draft** that claimed `ARMAndroidAuto` maps `HU_SAFEAREA_INFO` → AAP `content_insets`; that
field does not exist in this SDK.
...
**⚠️ SECURITY WARNING:** The `wifiName`, `btName`, and `oemIconLabel` fields are vulnerable to **command injection**. See `05_Security_Analysis/security_findings.md`.
...
|-----------------|---------------------|---------------|
| 0 (runtime value after reset on disconnect) | NOT started | Fails |
| 1 | Started | Works |
...
- `AndroidWorkMode` in riddle.conf: **persisted/config default is `1`** (per `--info`); may show `1` if set
- **Runtime state:** Always resets to `0` on phone disconnect / power-cycle (host must re-send `1` each session for Android Auto)
...
> **Note:** This 15,000 ms value is the heartbeat-receive watchdog (`0x3a98` at `0x21112`, per `heartbeat_analysis.md`). It is distinct from the **~60–70 s** "Host No Response" recovery latency reported in `usb_zlp_bug.md` for the USB write-stall path — a write stall does not trip the 15 s heartbeat watchdog because the adapter still reads heartbeats. The periodic reset watchdog is ~15,000 ms; there is **no separate ~60 s timer**. `Host No Response` is event-driven (fires on connection teardown / `ClosedByPeer`), and the ~60–70 s figure is a multi-cycle disconnect→recovery span (~4× the 15 s period), not a single watchdog.
...
| AndroidWorkMode | 1 | 1 | 5 | Protocol Init | Persisted/config default 1; runtime resets to 0 on disconnect/power-cycle (host must re-send 1 per session) |
| CarDrivePosition | 0 | 0 | 1 | Web UI |
| AndroidAutoWidth | 0 | 0 | 4096 | Protocol Init | **PASS-THROUGH in `ARMadb-driver`** (not read via its GetBoxConfig); read by `ARMAndroidAuto` via GetBoxConfig for AA video width (see AA viewarea section) |
| AndroidAutoHeight | 0 | 0 | 4096 | Protocol Init | **PASS-THROUGH in `ARMadb-driver`** (not read via its GetBoxConfig); read by `ARMAndroidAuto` via GetBoxConfig for AA video height (see AA viewarea section) |
...
| ImprovedFluency | Buffering behavior | **UNIMPLEMENTED** — no binary reads this value at runtime (fw 2025.10.15; see ImprovedFluency section above for the full binary analysis) |
