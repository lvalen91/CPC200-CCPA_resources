...
> **Scope note:** The 15,000 ms "Host No Response" constant documented here is the **heartbeat-receive watchdog** (fires when no inbound heartbeat is seen for 15 s). It is distinct from the **~60–70 s session-recovery latency** described in `usb_zlp_bug.md` for the USB write-stall path — that path keeps reading heartbeats, so the 15 s watchdog does not fire there. The periodic reset watchdog is ~15,000 ms; there is no separate ~60 s timer. `Host No Response` is event-driven (fires on connection teardown / `ClosedByPeer`), and the ~60–70 s figure is a multi-cycle disconnect→recovery span, not a single watchdog. The two share the "Host No Response" label but are different timers/contexts.
...
4. Heartbeat is host→adapter one-way (capture-verified): the adapter does not echo 0xAA, so disabling `SendHeartBeat` stops only the adapter's *outbound* emission — it does not relieve the host of its obligation to keep sending heartbeats
...
- Binary: `_evidence/cpc200_ccpa_firmware_binaries/unpacked/ARMadb-driver_unpacked`
