...
- `type=3` - **Microphone (upstream, host→adapter)**, `decodeType=5` (16 kHz PCM). `audioType=3` appears **only on the OUT/upstream direction** — it is the host's captured **microphone PCM**, active during Siri *and* phone calls. It is **not** "Voice (Siri)" downstream: the Siri/call voice you *hear* arrives as `audioType=1`/`decodeType=5` (16 kHz) on the IN/downstream path. `host_app_guide.md`'s `audioType 3 → sendMicrophoneData` is the correct interpretation.
- `type=4` - Phone call
