...
fun handleAudio(payload) {
    ...
    if (payload.size == 13) {
        ...
        when (audioType) {
            ...
            // audioType 3 = microphone (upstream, host→adapter), decodeType=5 / 16 kHz.
            // Active during Siri and phone calls; the host sends captured mic PCM on this value.
            // It is NOT the downstream Siri/call voice you hear — that arrives as
            // audioType=1 / decodeType=5 on the IN/downstream path. Routing audioType 3 →
            // sendMicrophoneData is correct.
...
fun handleAudioCommand(cmd: Byte) {
    when (cmd) {
        ...
        // NOTE: In this generic command-dispatch table, 0x04 = inputStop (the counterpart to
        // 0x03 inputStart). The 'Microphone Capture' block below uses a different, capture-derived
        // numbering in which PHONECALL_START is byte 5 (audio_protocol.md) / enum id 4
        // (carlink_native) — that "0x04" is NOT the same wire byte as this inputStop. The exact
        // wire byte for PHONECALL_START is still to be confirmed against the firmware.
        0x04 -> inputStop()
...
fun handleAudioCommand(decodeType: Int, cmd: Byte) {
    when (cmd) {
        // NOTE: this block uses capture-derived numbering, distinct from the generic
        // 'Audio Command Handling' table above (where 0x04 = inputStop). The wire byte for
        // PHONECALL_START is unconfirmed (byte 5 per audio_protocol.md vs enum id 4 per carlink_native).
...
**⚠️ SECURITY NOTE:** The `wifiName`, `btName`, and `oemIconLabel` fields are passed to shell commands via `popen()` without sanitization. This enables command injection - any shell command can be executed as root by including shell metacharacters in these fields. See `05_Security_Analysis/security_findings.md` for details.
...
fun sendBoxSettings() {
    val settings = JSONObject().apply {
        ...
        put("autoConn", true)
        // put("autoPlay", false)  // ⚠️ NOT MAPPED — ignored by ARMadb-driver (see § Media AutoPlay Implementation)
        put("bgMode", 0)
        put("startDelay", 0)
        ...
        // (see § Navigation Video Setup for the AdvancedFeatures / 508-kick details)
...
| 26 | `DashboardInfo` | `DashboardInfo` | cluster/dashboard capability bitmask (bit0 media-player, bit1 vehicle-status, bit2 route-guidance) |
...
(See `firmware_update.md` for the canonical SendFile wire format — 16-byte USB header, magic
`0x55AA55AA`, type `0x99`, `type_check` = complement of type, and the
pathLen/path+null/contentLen/content payload layout.)
...
See `05_Security_Analysis/security_findings.md` for complete security implications.
...
AirPlay `viewAreas` / `displayInfo` descriptor sent to the iPhone. **`ARMAndroidAuto` does NOT read
these files** — its aasdk openauto-v2 `VideoConfig` proto has only `margin_width`/`margin_height` (no
`ui_config`/`content_insets`), so Android Auto gets a symmetric letterbox margin only, sized from the
`androidAutoSizeW/H` keys — **not** from `HU_VIEWAREA_INFO` (whose only xref is `ProceessCmdOpen`). The
main-screen `HU_SAFEAREA_INFO` is **CarPlay-only** (binary-confirmed Jun 2026, firmware 2025.10: read
solely by `AppleCarPlay`; `ARMadb-driver` references `HU_VIEWAREA_INFO` + the cluster `HU_NAVISCREEN_*`
but not the bare main-screen SafeArea). A genuine AA content-inset SafeArea requires a host speaking
the AAP protocol directly, not the dongle — see `01_Firmware_Architecture/configuration.md` § "Android
Auto: margins only — no main-screen SafeArea".
...
**Key distinction:** `IsAutoPlayMusic` is NOT sent to the adapter. It is purely a host-app preference that controls whether the host sends Command 201 on reconnect.
Since the BoxSettings `autoPlay` → `AutoPlauMusic` mapping is broken (see above), this host-side mechanism is the **only reliable way** for a host app to implement auto-play over USB.
...
1. **Enable GPS capability on adapter** (one-time, requires SSH or command injection):
   ...
   **GNSSCapability bitmask:** bit 0=GPGGA(1), bit 1=GPRMC(2), bit 3=PASCD(8). Use value `3` for GPGGA+GPRMC. `HudGPSSwitch` defaults to `1` (already correct).
`DashboardInfo` does NOT need to be changed for GPS — it is the cluster/dashboard capability bitmask (bit0 media-player, bit1 vehicle-status, bit2 route-guidance), not location.
...
- Source: `GM_research/_evidence/cpc200_research/docs/implementation/`
