...
(This capture-backed statement is the verified counterpart to the design-guidance note in
`04_Implementation/host_app_guide.md`.)
...
In this (wired) CarPlay session, navigation video was activated by sending `naviScreenInfo` in BoxSettings, and the adapter emitted the 508 nav-focus handshake on its own. `AdvancedFeatures=1` is not required when `naviScreenInfo` is provided — see `04_Implementation/host_app_guide.md` (navigation-video section) for the canonical statement.
...
| 55 | 39319ms | IN | 163 | SessionToken | Encrypted blob (444 bytes) |
...
| `bssid` | DE:AD:BE:EF:00:02 | Adapter's WiFi MAC |
...
[BoxRFCOMMService] initWifiInfo, channelfreq: 5180, channeltype: 0,
  ...
  bssid: DE:AD:BE:EF:00:02, securityMode: 8
...
- **CarPlay:** Host sends `naviScreenInfo` in BoxSettings → adapter negotiates second video stream (for wireless CarPlay a proactive 508 kick is also required — see § *Navigation Video Activation*)
...
[OpenAuto] [BluetoothService] sending local adapter address: DE:AD:BE:EF:00:05
