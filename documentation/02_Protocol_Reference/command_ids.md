...
| 508 | 0x1FC | RequestNaviScreenFocus | BOTH | Three-step nav-screen-focus handshake; on wireless CarPlay the adapter never initiates, so the host must send first (508 also kicks the iOS nav video encoder). Canonical detail in `command_details.md`. |
