...
call fcn.000197a8(r6, r5);               // Command dispatch/forward helper (the command-ID→string-name lookup is the separate function at 0x19244)
```
...
| ID Range | Group | Summary |
|----------|-------|---------|
| 500-509 | Focus | RequestVideoFocus, ReleaseVideoFocus, audio focus variants, NaviScreen focus (508/509). Mainly Android Auto, but 506/507 and 508/509 are also used by CarPlay (wired + wireless) |
| 1003-1011 | Connection Status | WiFi/BT scan results, connect/disconnect notifications |

**⚠️ CRITICAL (Binary Verified Feb 2026):** Command 1010 (`DeviceWifiNotConnected`) is a **WiFi hotspot status notification, NOT a session termination signal** — the host should log status / update its UI indicator but **must not terminate active sessions**. For the full interpretation (binary evidence, when it is sent, and USB-vs-Wireless handling), see `command_details.md` (canonical) and the [Command 1010 Binary Analysis Evidence (Feb 2026)](#command-1010-binary-analysis-evidence-feb-2026) section below.
...
**Source:** Direct disassembly of `ARMadb-driver.unpacked` command-name-lookup function at `0x19244` (`0x19744` is an interior string-pointer literal of this function, not a separate function)
...
### Focus Commands (500-509) — Verified Jan 2026

These commands manage audio/video/navigation focus. 500-505 appear mainly in Android Auto; 506/507 (audio nav focus) and 508/509 (NaviScreen video focus) are also used by CarPlay — both wired and wireless:
...
| 508 | 0x1FC | RequestNaviScreenFocus | BOTH | Three-step nav-screen-focus handshake; on wireless CarPlay the adapter never initiates, so the host must send first (508 also kicks the 0x2C nav-screen video stream). Canonical detail in `command_details.md`. |
...
|---------|----------|---------|
| `0x19244` | Command name lookup | Switch over command IDs → string name (entry `0x19244`, size 1150, ≈29 callers). **`0x19744` / `0x197a8` are NOT functions** — they are interior literal-pool string-pointer constants of this function (`0x19744`→"CtrlButtonDown", `0x197a8`→"PhoneKey9"). |
...
| 21 | 3 | `e8 07 00` | Additional data, making the full payload 24 bytes (interpretation unconfirmed — possibly year: 2024) |
...
The command ID → string name mapper is the function at `0x19244` (`0x19744` is one of its interior string-pointer literals, not the entry). Disassembly shows:
...
```json
"DevList": [
  {"id":"DE:AD:BE:EF:00:03", "type":"CarPlay", "name":"lePhone"},
  {"id":"DE:AD:BE:EF:00:08", "type":"CarPlay", "name":"Matt"}
]
```
