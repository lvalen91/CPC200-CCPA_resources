...
**IMPORTANT:** The `wifi` field determines USB vs Wireless connection, NOT phoneType. (Offset `0x04` is the transport `wifi` flag — `0` over USB. A `connected=1` label seen on the same field in a wired `usb_protocol.md` capture is that field mislabeled, not a separate value.)
...
These 14 BT / 16 WiFi figures are **advertised-per-pass** counts. They are distinct from the
**total registered** iAP2 component set (19, per `carkit_ios26_master.md`), which includes
transport/HID/availability components that are not all emitted in every pass. The per-pass vs
total-registered reconciliation is still open (see FLAG #8 below).
...
| `box.ver` | Firmware version (2025.10-family CCPA release; this capture ran `2025.10.15.1127`. The `2025.02.25` build cited in `carplay_handshake.md` is a different but equally genuine CCPA firmware release — both are real, not a conflict) | e.g., 2025.10.15.1127 |
