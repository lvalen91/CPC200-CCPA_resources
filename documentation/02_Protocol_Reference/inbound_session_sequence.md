...
|---------|---------|---------|
| `Command(DVRCommand_RequestPreview)` | 0x08 cmd=600 | Originally logged here on Home/CarPlay button press. Binary-verified tables (`command_ids.md`, `command_details.md`) establish cmd 600 = DVRCommand_RequestPreview; RequestHostUI is command 3 (0x03). |
| `Command(DeviceWifiNotConnected)` | 0x08 cmd=1010 | WiFi status ping (informational, not session end). Originally logged here as cmd=1006, but binary-verified tables map 1006 = DeviceConnectFailed; the WiFi-not-connected status is cmd 1010. |
...
4. **WiFi-not-connected status is informational** — `Command(DeviceWifiNotConnected)`
   (cmd 1010, NOT 1006) does NOT indicate session end. Real disconnects come via
   `Unplugged` (0x04) or `Phase(0)`. See `command_details.md` (command 1010) for the
   canonical analysis.
