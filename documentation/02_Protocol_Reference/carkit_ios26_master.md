...
The bare enumeration of these `supports*` / `setSupports*` symbols (62 total) is mirrored in `iap2_sdk_delta_r14g17_to_ios26.md`, which is the canonical flat capability list; this matrix is the authoritative home for the per-flag unlock semantics, semantic domain, and CCPA-claim columns. The two files diverge only on the CCPA-claim column for `supportsUSBCarPlay`, `supportsMixableAudio`, `supportsThemeAssets`, and `supportsMapContent` — this file's values (the verified/most-recent multi-agent consolidation) are authoritative; sdk_delta's differing CCPA-claim entries are retained there as lower-confidence dissent.
...
Based on observed iPhone behaviour toward a stock CPC200-CCPA running ARMiPhoneIAP2 firmware version 2025.10. Agent 1 inference + Agent 3 cross-check against the firmware RE doc (the (offline) ARMiPhoneIAP2 binary-RE notes — deferred, not yet integrated).
...
- `supportsMutualAuthentication` — backed by real MFi coprocessor at I2C-1 0x11 (per internal session notes)
- `supportsAudio`
...
This 19 is the **total registered** component set. A separate identification capture (`device_identification.md:195-196`) counts **14 components advertised in the BT identification pass** and **16 in the WiFi re-identify pass** — those are advertised-per-pass counts, not the total-registered set (transport/HID/availability components are not all emitted in every pass). Whether 14/16-per-pass and 19-total fully reconcile is still open (see FLAG #8 / report `#c8`); not yet ground-truthed.
...
4. Patch by branching to the code cave (the **1224-byte** zero-filled gap at file `0x76b38` / VMA `0x86b38`).
5. Re-LZMA-pack the binary (with the modified ARM UPX, on-device) and deploy.

Same code cave + toolchain as the deployed NAV/CALL/COMM patches — see [`../04_Implementation/iap2_binary_patches.md`](../04_Implementation/iap2_binary_patches.md) for the full mechanism (cave layout, hooking model, unpack→patch→repack pipeline, deploy gate, rollback). *(The "1944-byte cave at `0x86868`" figure in earlier internal notes was wrong; the device-verified cave is 1224 bytes at `0x86b38`.)*
...
- Internal session notes — the original three-layer pipeline finding (this session)
- Internal session notes — Bug A history that drove discovery of the cursor field
- Internal session notes — real MFi coprocessor at I2C-1 0x11 verifying `supportsMutualAuthentication`
...
### 11.3 CPC200-CCPA prior RE assets

- the (offline) ARMiPhoneIAP2 binary-RE notes (`iap2_binary_analysis.md`) — ARMiPhoneIAP2 binary RE, RouteGuidance JSON builder, the 19-component identification list, string-table mapping — deferred, not yet integrated
- ARMiPhoneIAP2 firmware-patching work — **now integrated** in [`../04_Implementation/iap2_binary_patches.md`](../04_Implementation/iap2_binary_patches.md) (NAV/CALL/COMM raw-passthrough patches: code cave, hooks, pipeline, deploy gate)
- `_evidence/aa_rebuild/adapter_memdumps_20260319/ARMiPhoneIAP2_449_code.bin` — live binary code dump from adapter
- `_evidence/aa_rebuild/adapter_memdumps_20260319/ARMiPhoneIAP2_449_data.bin` — data section dump
- `_evidence/cpc200_ccpa_firmware_binaries/2025.02/NAVIGATION_PROTOCOL_ANALYSIS.md` — earlier protocol analysis
- `_evidence/cpc200_ccpa_firmware_binaries/PROTOCOL_ANALYSIS.md` — general adapter protocol
- `_evidence/cpc200_ccpa_firmware_binaries/analysis/iap2_runtime_full.bin` — runtime memory full dump
- `_evidence/cpc200_ccpa_firmware_binaries/memdump_20260521/iap2_504_{rw,rx,heap}.bin` — partitioned memory dumps
...
- the 2017 CarPlay SDK (R14G17) Integration Guide (`AppleCarPlay_CommunicationPlugIn_IntegrationGuide.txt`)
- the 2017 CarPlay SDK (R14G17) AccessorySDK source (AirPlay layer)

### 11.5 iOS 26.5 extracted frameworks (this session)

- the iOS 26.5 `CarKit` framework — host-side CarPlay framework
- the iOS 26.5 `AccessoryNavigation` framework — navigation/iAP2 wrapper
- the iOS 26.5 `CoreAccessories` framework — master iapd library
- the iOS 26.5 `AccessoryiAP2Shim` framework — XPC bridge
- the iOS 26.5 `ExternalAccessory` framework — public EA framework
- the iOS 26.5 `CoreAccessoriesFeatures` framework — AudioProduct cert handler (NOT a general feature framework, despite name)
- the iOS 26.5 `CarKitNavigation` framework — CarKit nav consumer of `ACCNavigationAccessoryComponent`
...
|-------|-------|--------------------------|
| Agent 1 | CarKit framework deep RE | Full 60+ supports* / setSupports* list (mirrored as a flat 62-entry list in `iap2_sdk_delta_r14g17_to_ios26.md`; see Section 3 note). CCPA inferred-claim inventory. CRCarPlayCapabilities persistence mechanism + 17 dictionary keys. `disabledFeature` u64 bitmask channel. Night-mode resolution priority. Two-tier mutual-auth flag. |
