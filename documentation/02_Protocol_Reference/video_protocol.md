...
**Observed Values (CarPlay — wireless capture; see the wired correction below):**
...
| 3 | 0x00000003 | 1 | This table is a **wireless** capture (value 3 is rare/transient here). On **wired** CarPlay, EncoderState=3 is the steady-state — see correction below. |

_This frequency table is from a **wireless** session. On **wired** CarPlay main video, EncoderState=3 is the steady-state (after a brief startup value); on **wireless**, 3 does not appear at all — large encoder-counter values dominate, and the specific value varies between wireless sessions. Treat EncoderState as session/transport-dependent, not a fixed codec selector._
...
**Correction:** The "CarPlay=7 / Android Auto=3" split is incomplete — wired CarPlay main video is `encoderState=3` for all frames while wireless CarPlay main is `7`; nav (0x2C) is `1` constant on both. Decoding this field as a codec selector (the `bits2-3 = 1:H264 / 2:MJPEG / else:H265` rule used in some host code) **misclassifies `3` as H.265**, but the bitstream is unambiguously H.264 (SPS profile=100) in every case. **Do not pick a decoder from encoderState — parse the SPS.** The only reliable H264-vs-other bit in this dataset was bit2 (set on wireless `7`, clear on wired `3`), not the documented 2-bit field.
...
**Requirements (Corrected Feb 2026; partly superseded by live host A/B test 2026-06-13 — see Verified findings below):**
- iOS 13+
- Host must send `naviScreenInfo` in BoxSettings JSON (USB type 0x19) — advertises cluster geometry. Firmware parses it at `0x16e5c`. For the BoxSettings wire layout see `CAPTURE_SESSION.md` §9.1.2; for the navigation-screen geometry path see §9.4.
- `AdvancedFeatures=1` persistent unlock **IS required** (see Verified findings). The earlier "NOT required / disproven by testing" note was misleading — it held only because that test adapter had already been unlocked at least once in its lifetime. On a not-yet-unlocked adapter, `naviScreenInfo` alone does nothing.
- Command 508/509 are the focus gate (see Verified findings).

**Verified findings (live host A/B test — carlink_macOS host, CPC200-CCPA FW 2025.10.15.1127, wired CarPlay):**

These findings come from a single-process host implementation:

1. **`AdvancedFeatures=1` is a hard prerequisite.** With `naviScreenInfo` present in 0x19 AND Command 508 sent, but the adapter not yet unlocked, the adapter **never echoed 508 and emitted zero 0x2C frames** — it only sent 506/507 (audio nav focus). After running `riddleBoxCfg -s AdvancedFeatures 1 && riddleBoxCfg --upConfig` on the adapter, 0x2C began. The gate is the firmware reading persistent `HU_NAVISCREEN_INFO` into `g_bSupportNaviScreen` at AppleCarPlay session-init.

2. **A full adapter power-cycle/reboot is NOT required — but a fresh session-init IS.** Setting `AdvancedFeatures=1` mid-session had no effect, and a phone unplug/replug alone also failed to trigger it. What worked: a **host re-initialization** (host app relaunch → USB re-enumeration → new init sequence + fresh 0x19) followed by the CarPlay session re-establishing. Re-initialization, not a reboot, is the operative requirement once the unlock file exists.

3. **508 starts and 509 stops the stream — confirmed bidirectional gate.** Once unlocked, the adapter **initiates** Command 508 (`RX cmdId=508`) right as nav streaming begins; the host acks with 508 and 0x2C flows. Sending **509 (ReleaseNaviScreenFocus) stops 0x2C within ~seconds** — the iPhone's alt-screen encoder actually turns off (main video 0x06 continues uninterrupted). So 508/509 are genuine on/off control of the iOS-side encoder, not merely adapter-side muting.

4. **Stream characteristics observed:** 0x2C delivered at the host-configured resolution (1280×720 @ 24 fps requested → honored), H.264 **High profile level 3.1** (`27 64 00 1f`), with the documented 36-byte header. Decoded directly by the host (VideoToolbox / AVSampleBufferDisplayLayer) into a separate window — no AIDL/IPC forwarding needed for a single-process host.

5. **Wired AND wireless both work — support is gated only by the unlock, not the transport.** Both wired and wireless CarPlay sessions produced 0x2C + the nav window once the adapter was unlocked; the only zero-0x2C sessions were the pre-unlock ones (incidentally wired). The documented 508-initiation difference is **confirmed, not contradicted**: on **wired** the adapter sends 508 first and the host echoes; on **wireless** the adapter does NOT initiate — the host must send 508 proactively (this host auto-sends it ~2 s post-PLUGGED), and the adapter then **replies** with 508 (observed ~0.8–9 s later), after which the host echoes and 0x2C flows. A host that BOTH proactively sends 508 post-PLUGGED AND echoes any received 508 works transparently across both transports — no transport-specific code needed. (Note: this also confirms wireless 0x2C reaches a non-AAOS USB host directly — the adapter re-frames the AirPlay type-111 stream as USB 0x2C exactly as documented.)

**Verified findings — macOS-host binary capture analysis (wireless + wired, adapter FW 2025.10.15.1127CAY):**

> **PROVENANCE / STALENESS:** these findings derive from the **carlink_macOS** host app (single-process macOS host, VideoToolbox decode), analyzing two on-device binary session captures. Adapter-side behavior is shared with the Android host, but **host-side specifics may differ from carlink_native (Android)** — some quirks/comments elsewhere in this tree are Android- or firmware-revision-specific. More broadly, this RE doc tree accumulates entries from many test points across both macOS and Android host development and multiple adapter firmware revisions; **older/undated entries may be stale or host-specific.** Prefer dated "Verified findings" blocks.

1. **Main video (0x06) is keyframe-starved by design.** It emits one IDR at stream start, then runs on pure P-slices indefinitely (captured: 0 IDR in 84 s wireless — capture began after the startup IDR; 1 IDR in 139 s wired @ ~1.3 s, none after). Nav (0x2C) ≈ 1 IDR / 26 s on wired, sparser on wireless. **Implication:** a host decoder cannot self-recover from frame loss/reset by waiting — it must **explicitly request a keyframe** (cmd 12 keyFrame / refresh) after any loss, decoder reset, or window re-create. (Tearing down + recreating a nav decoder mid-stream yields a black window until the next, rare, IDR — observed and fixed on the macOS host by hiding rather than destroying the decoder.)

2. **encoderState does NOT reliably indicate codec** — see the boxed correction in the EncoderState section above (wired CarPlay main = 3, contradicts the CarPlay=7 split; `bits2-3` decode misclassifies 3 as H.265; sniff the SPS instead).

3. **0x2C idle cadence is content-driven, not transport/link.** Static screen → ~1 frame / 4–6 s keep-alive with tiny ~2 KB P-frames; animating → identical ~42 ms (~24 fps) median on BOTH wired and wireless. Nav inter-frame gap is therefore NOT a link-quality/transport signal (resolves an earlier "wireless smoother than wired" impression — it was activity, not the link).

4. **Idle signature (per channel)** — useful for programmatic idle detection. At true idle homescreen: 0x06 ≈ off (~0.3 fps); 0x2C continues ~15–20 fps but frame size collapses ~8× (~2 KB); audio 0x07 **entirely absent** (no silence keepalive); touch 0x17 / metadata 0x2A / most 0x08 = zero; heartbeat 0xAA steady ~2.0 s. Cleanest gate: 0x06 fps ≈ 0, or median 0x2C frame < ~3 KB.

5. **0x08 command is bidirectional.** The adapter sends commands TO the host (RX) — on wired CarPlay too: 18/19 (GNSS start/stop), 506/507 (audio nav focus request/release). Host→adapter (TX) examples: 12 (keyFrame), 508 (nav-screen focus). Treat 0x08 as a bidirectional command channel.

6. **0x2A is a plaintext JSON telemetry channel** (RX): leading u32 subtype + null-terminated JSON. Subtype **1** = Media (`MediaSongPlayTime`, `MediaPlayStatus`), **200** = Navi turn-by-turn (`NaviStatus`, `NaviTimeToDestination`, `NaviDestinationName`, `NaviDistanceToDestination`, `NaviRemainDistance`, `NaviRoadName`…). Navi fields can arrive **combined in one packet** (not always two separate sends). Also: audio 0x07's `volume` field read 0.0 on every data frame (appears unused on the data stream).

7. **508 wireless reply latency measured ≈ 855 ms** (host TX 508 → adapter RX 508), within the documented 0.8–9 s range; reconfirms wired adapter initiates 508 unprompted, wireless only replies after the host's proactive 508.

**Transport-independent (not to attribute to wired/wireless):** init sequence (byte-identical order/timing), time-to-first-main-frame (~3.5 s), nav active cadence (~24 fps), and both H.264 profiles. **Confounds in this dataset:** main bitrate (= on-screen motion), audio presence (the wired session simply had no media playing — cannot disambiguate content vs wired-audio routing here), and nav start time (= the user's nav-enable toggle).
...
**Wireless nav video DOES appear on USB as 0x2C.**

On wireless CarPlay the adapter re-frames the AirPlay type-111 stream as USB 0x2C — it does **not** transcode the H.264 payload, it only re-wraps the existing bitstream in USB 0x2C framing. A macOS-host wireless capture contained **1,469 genuine 0x2C frames** (720×1280, H.264 High@3.1) over USB, identical to wired.
...
**Note:** The H.264 level is **negotiated per session**, not hardcoded by the adapter. The iPhone advertises up to `h.264Level5.1` in the AirPlay SETUP `features` array (a *capability* flag, listed beside `cornerMasks`/`mainBuffered`); the level actually encoded depends on the resolution/FPS chosen for that session.
The 2026-05-21 capture (2400×960 @ 60 FPS) encoded **Level 5.0**: avcC `01 64 00 32` and the SPS NAL `27 64 00 32` both carry `profile_idc=0x64` (High) and `level_idc=0x32` (=50, Level 5.0); no `level_idc 0x33` (5.1) SPS exists in the capture. The `h.264Level5.1` in `carplay_handshake` is the iPhone's advertised capability, **not** the encoded level. Level 5.0 supports up to 4096×2304 @ 30fps, providing headroom for 2400×960 @ 60fps.
...
The 508 (RequestNaviScreenFocus) nav-focus handshake is host-initiated and three-step: the host sends 508, the adapter replies with 508, and the host echoes 508 to confirm; the adapter then emits the `HU_NEEDNAVI_STREAM` D-Bus signal and Type 0x2C (NaviVideoData) frames begin. On **wireless** CarPlay (adapter WiFi AP) the adapter never initiates — it sends only 506 (audio nav focus) — so the host must send the first 508 itself (~2 s after PLUGGED, after `naviScreenInfo` BoxSettings has been sent). On **wired** sessions the adapter has historically initiated (echo-only host code works there). Transport-agnostic code should send 508 proactively AND echo any incoming 508 — repeated 508s are idempotent.

**Canonical source:** the full three-step 508 handshake (step-by-step table, wire-sequence diagram, idempotency, wired-vs-wireless behavior, live-verification stamp on firmware 2025.10.15.1127) is documented in `02_Protocol_Reference/command_details.md` (Command 508 / RequestNaviScreenFocus, BOTH-direction). This section retains only the navigation-video–specific context below.

If step 1 is omitted on a wireless session, the adapter never replies (it sends only 506) and the handshake never starts. If the host echo (step 3) is omitted, the adapter accepts the first 508 but doesn't reach the emit path — `_AltScreenSetup` does not complete.
...
> **Note:** The D→H rows `110`/`111` above are a host-side nav-focus enum and do **not** correspond to the canonical USB `0x08` Command IDs. In the binary-verified command tables (`command_ids.md`, `command_details.md`) IDs `107–110` are an unused gap and `111` (0x6F) is `CtrlKnobLeft` (rotary knob counter-clockwise, H→A→P). Treat 110/111 in this table as a separate namespace, not the canonical command IDs.
