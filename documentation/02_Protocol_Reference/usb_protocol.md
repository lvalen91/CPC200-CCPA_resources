...
| 100 | 0x00000064 | CallStatus JSON (phone call state); `_callraw` raw `0x4155` frame added by the CALL patch | 17-130 bytes |
| 101 | 0x00000065 | CommunicationUpdate JSON `_commraw` (raw `0x4158`) — **patch-only**, emitted by the COMM patch (stock firmware never sends subtype 101) | 11-80 bytes |
| 200 | 0x000000C8 | Navigation JSON (route, TBT directions); `_iap2`/`_iap2m` raw frames added by the NAV patch | 30-200 bytes |
| 201 | 0x000000C9 | Navigation maneuver icon (AA-only PNG) | 1739-2542 bytes |

> Subtypes 100/101 and the `_callraw`/`_commraw`/`_iap2` raw-frame keys come from the iAP2
> raw-passthrough binary patches — see
> [`../04_Implementation/iap2_binary_patches.md`](../04_Implementation/iap2_binary_patches.md) and
> field semantics in [`iap2_message_catalog.md`](iap2_message_catalog.md). Subtype 101 appears only
> when the COMM patch is deployed.

**Packet Structure:**
```
Offset  Size  Field        Description
------  ----  -----        -----------
0x00    4     Subtype      Content type indicator (LE uint32)
0x04    N     Content      JSON string, JPEG, or PNG data
```
...
**CallStatus JSON (Subtype 100 / 0x64) — Discovered 2026-04-11, Live Capture Verified (wired + wireless CarPlay, iPhone 18,4):**

Sent by the adapter's `CiAP2CallStateEngine` during phone calls. Provides structured call metadata including caller identity — richer than audio commands alone.
...
```json
{
  ...
  "CallName": "Jenny",
  "CallNumber": "+15758675309"
}
```

| Field | Type | Description |
|-------|------|-------------|
| `CallStatus` | int | 0=ended/idle, 1=dialing(out), 2=ringing(in), 3=connecting(brief), 4=active(long-lived) — **corrected 2026-06** (was "3=connected, 4=disconnecting"); see [`iap2_message_catalog.md`](iap2_message_catalog.md) |
| `CallDirection` | int | 1=incoming, 2=outgoing |
| `CallID` | string | UUID v4, unique per call — correlates all status transitions |
| `CallName` | string | Contact name from iPhone address book (or dialed number if no match) |
| `CallNumber` | string | Full E.164 phone number with country code (e.g., "+15758675309") |

**Minimal form:** `{"CallStatus":0}` (17B) — sent at session start and when no call is active. No CallDirection/CallID/CallName/CallNumber fields.

**State machine (corrected 2026-06 from `_callraw` device verification):**
```
         Outgoing:  0 → 1 (dialing)  → 4 (active) → 0 (ended)
  Incoming answer:  0 → 2 (ringing)  → 3 (connecting) → 4 (active) → 0 (ended)
  Incoming reject:  0 → 2 (ringing)  → 0 (ended, skips 3/4)
Incoming voicemail: 0 → 2 (ringing)  → 0 (ended, skips 3/4, after ~20s ring)
```
`3` is a brief post-answer "connecting" step; `4` is the long-lived **active** state (observed
persisting ~71 s on a live call); `0` is the end. The earlier labels "3=connected/4=disconnecting"
were a misread.

**Timing relationship with audio commands:**
- Audio commands (INCOMING_CALL_INIT, PHONECALL_START/STOP) are the primary session triggers
- CallStatus JSON arrives ~300ms AFTER the corresponding audio command
- CallStatus provides supplementary metadata (caller name/number/direction) not available in audio commands
- CallStatus=0 (idle) is the definitive end signal — more reliable than duplicate PHONECALL_STOP audio commands

**Payload sizes:** 17B (idle-only) / ~119B (outgoing) / ~130B (incoming, longer phone number field)

**DashboardInfo independence:** CallStatus subtype 100 is generated by `CiAP2CallStateEngine`, which is **always enabled** regardless of the `DashboardInfo` bitmask value (verified in firmware config docs). All adapters will send CallStatus regardless of DashboardInfo setting. Captures were performed with `DashboardInfo=5` (bits 0+2: MediaPlayer + RouteGuidance, no LocationEngine bit 1) — CallStatus was received normally.
...
0x04    4     wifi         Transport flag: 0=USB, 1=Wireless (per device_identification.md)
```

> **Offset 0x04 — field semantics (reconciled #c18):** `device_identification.md` is authoritative: offset 0x04 is the **wifi transport flag** (0=USB, 1=Wireless) — the discriminator for USB vs Wireless CarPlay. Earlier usb_protocol notes labeled this field "connected"; one wired USB capture reported it as `1` (see USB Capture Timeline below), which is unexpected for a pure wifi flag (USB should read 0). Whether Plugged additionally carries a distinct "connected" flag is unconfirmed.
...
**⚠️ SECURITY WARNING:** The `wifiName`, `btName`, and `oemIconLabel` fields are vulnerable to **command injection**. These values are passed to `popen()` shell commands without sanitization. See `05_Security_Analysis/security_findings.md` for details.
...
```json
{
  ...
  "HiCar": 1,
  "WiFiChannel": 36,
  "DevList": [{"id": "DE:AD:BE:EF:00:06", "type": "CarPlay"}]
}
```
...
0.409s    IN   BluetoothPairedList   "DE:AD:BE:EF:00:06User"
...
...
2.362s    IN   PeerBluetoothAddress  "DE:AD:BE:EF:00:06"
3.645s    OUT  HeartBeat             (first heartbeat)
3.662s    IN   PeerBluetoothAddressAlt
4.180s    IN   Plugged (0x02)        phoneType=3, 0x04=1 (labeled "connected"; see 0x04 note)
...
> **Source:** Binary dump from adapter `/proc/PID/mem`, strings analysis at `a strings dump of the unpacked ARMAndroidAuto binary`
...
**What earlier CarLink Native testing showed:**
- On wired sessions, navigation video worked with `naviScreenInfo` configured, so on that path the 508 handshake's necessity could not be isolated as the sole factor. For wireless CarPlay the 508 kick is required (see above).

**Recommendation:** Always echo 508 back when received, and proactively send it for wireless CarPlay nav video. `naviScreenInfo` in BoxSettings configures the nav screen geometry but does not, by itself, start the wireless nav video encoder.
...
| 0x2A | DashBoard_DATA | Variable | Parse subtype: 1=media JSON, 3=album art, 100=CallStatus JSON (`_callraw` if CALL patch), 101=comm JSON `_commraw` (COMM patch only), 200=NaviJSON, 201=NAVI_IMAGE (AA-only PNG maneuver icon). Update media session, call state, and navigation state. |
