...
| USB Protocol Payload | AES-128-CFB128 | USB message payload encryption (key: `SkBRDy3gmrw1ieH0`) |
| SessionToken | AES-128-CBC | Session telemetry encryption (key: `W2EC1X1NbZ58TXtn`) |
| Firmware | AES-128-CBC | Firmware image encryption |

---

## Three Distinct Encryption Systems (IMPORTANT)

The CPC200-CCPA uses **three completely separate encryption systems** that should not be confused:
...
| **Algorithm** | AES-128-CFB128 (via OpenSSL `AES_cfb128_encrypt`) |
...
| **Exempt Types** | See "Encryption Bypass" per-type table below |
| **Magic Marker** | See "Protocol Encryption Parameters" below |
| **Toggle** | Type 0xF0 zero-payload ack from adapter |
| **HW Accel** | See "Hardware AES Engine" section |

The protocol payload key is `SkBRDy3gmrw1ieH0` (AES-128-CFB128). The `W2EC1X1NbZ58TXtn` key is SessionToken-only and the cipher is not AES-128-CTR. See the dedicated "USB Protocol Payload Encryption" section below for full detail.
...
| `SkBRDy3gmrw1ieH0` | AES-128-CFB128 | USB protocol payload encryption (runtime) |
...
> **Binary key locations:** In the unpacked `ARMadb-driver`, the key strings sit at `SkBRDy3gmrw1ieH0` → VMA `0x6d0d4` and `W2EC1X1NbZ58TXtn` → VMA `0x6dc7b`, and the magic markers `0x55AA55AA` (cleartext) / `0x55BB55BB` (encrypted) appear as code immediates. The protocol payload AES path uses OpenSSL `AES_cfb128_encrypt`.
...
A fresh wireless CarPlay pairing was captured (`_evidence/MFi_research/capture/carplay-20260521-101016/`).
...
| **Register map** | See `_evidence/MFi_research/docs/register_map.md` |
...
## AirPlay Pairing Identity Storage

The adapter's **AirPlay accessory pairing identity** — the long-term **Ed25519 keypair** used for the HomeKit pair-setup / pair-verify on the AirPlay side (distinct from the hardware MFi IC above and from the `/var/lib/lockdown/` USB host-pairing records) — is stored in the keychain `/Library/Keychains/default.keychain`, under service `com.apple.airplay.pairing`:

| Keychain label | Contents |
|----------------|----------|
| `AirPlay Pairing Identity: <UUID>` | the accessory's own Ed25519 keypair (the value data) |
| `Paired AirPlay Accessory: <UUID>` | one record per paired iPhone/device |

Each `<UUID>` is device-specific. Deleting or renaming this keychain (e.g. `mv /Library/Keychains/default.keychain /Library/Keychains/default.keychain.bak`) discards the identity and forces fresh pairing — see `02_Protocol_Reference/wireless_carplay.md` (Force Re-Pairing).
...
## USB Protocol Payload Encryption (AES-128-CFB128)

The protocol payload encryption uses **AES-128-CFB128** with key **`SkBRDy3gmrw1ieH0`**. (An older analysis labeled this AES-128-CTR with key `W2EC1X1NbZ58TXtn`; that was incorrect — `W2EC1X1NbZ58TXtn` is the SessionToken key only.)

### Two Distinct Runtime Encryption Keys

| Key | Mode | Purpose | Location |
|-----|------|---------|----------|
| `SkBRDy3gmrw1ieH0` | AES-128-CFB128 | Protocol payload encryption (all non-exempt types) | `0x6d0d4` in unpacked binary |
...
| Parameter | Value |
|-----------|-------|
| **Algorithm (Firmware)** | AES-128-CFB128 (via OpenSSL `AES_cfb128_encrypt`) |
...
| **HW Acceleration** | `/dev/hwaes` — see "Hardware AES Engine" section |

#### Cipher Mode: CFB128 (firmware and app)

Both ends use **AES-CFB128**, which is why they interoperate. The firmware's AES path runs through OpenSSL `AES_cfb128_encrypt`, and the AutoKit app uses `Cipher.getInstance("AES/CFB/NoPadding")` (Java source line 2539). (An older analysis labeled the firmware side `AES_cbc_encrypt`; the cipher is in fact CFB128.)

> Caveat: because `AES_cbc_encrypt` is not actually called in `ARMadb-driver`, the SessionToken "CBC" label in the same binary is also suspect and should be re-checked. The firmware-image mode lives in the separate `ARMimg_maker` binary and was not re-traced.

**Recommendation:** Use **AES/CFB128/NoPadding** for the USB protocol payload (matches both firmware and app). The key, IV, and exempt-type logic are unchanged.
...
AES_cfb128_encrypt(payload, payload, len, schedule, iv, direction)  ; at 0x17ee4
```
...
| All others | - | **YES** | Payload encrypted via AES-128-CFB128 |
...
AES_128OFBDecrypt()
// AES-CFB128 support found in AppleCarPlay binary — and used by the AutoKit app and USB protocol payload
```
...
| Path | Purpose |
|------|---------|
| `/dev/hwaes` | Hardware AES acceleration (misc device 10:0, confirmed on live firmware) |
