...
| Size | 3,407,872 bytes (3.3MB) — kernel binary length (the 0x4F0000/~5MB figure in the U-Boot read below is the read/partition window spanning kernel-plus-padding up to the DTB, not the binary size) |
| Flash Offset | 0x100000 (1MB from start) — corrected to match the authoritative U-Boot norboot read below; the earlier 0x40000 (256KB) value was the mtd0/U-Boot partition size mislabeled as the kernel offset |
...
| `W2EC1X1NbZ58TXtn` | AES-128-CBC | Failed - random output |
...
sf read 0x80800000 0x100000 0x4F0000    # Read kernel from flash offset 0x100000 to RAM (0x4F0000/~5MB read window spans the 3.3MB kernel plus padding up to the DTB)
...
Source: `_evidence/MFi_research/CAPTURE_SESSION.md` §2, §8.
