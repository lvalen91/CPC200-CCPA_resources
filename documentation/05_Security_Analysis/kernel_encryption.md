...
**2026-06-30 late night — retried with the RAM dump as a verification oracle, found one more real (harmless) bug, same completion race persists:** Reasoned that since u-Boot's own disassembled call uses a *single* descriptor for the entire 3,358,720-byte transfer, our chunked (64 KB) submissions might themselves be the problem — so tried passing `chunk_size == total_len` to force one whole-file descriptor. This immediately crashed the on-device process (clean segfault, zero effect on the running kernel/hardware, confirmed via unbroken uptime + clean `dmesg` both times): the tool's scratch data buffer lives inside a *fixed* `mmap()` of only `OCRAM_LEN` (`0x1B000`) bytes, of which just the last `0x10000` (64 KB) is usable as the data buffer — `memcpy`ing 3.3 MB into it writes straight past the mapped region into unmapped memory. u-Boot can do a true single descriptor because it decrypts in place in real System RAM, not a small OCRAM scratch buffer; our tool cannot without a redesigned (much larger) buffer strategy. **Fixed:** `dcp_decrypt.c` now clamps any requested chunk size to the actual OCRAM buffer capacity and warns instead of overrunning. Re-ran the corrected tool with the real full mtd1 ciphertext (3,358,720 bytes, default 64 KB chunking, exactly like the tool's original design) — got the same clean chunk-0 timeout as every previous careful attempt (`stat=0x10000000`, i.e. the low nibble the poll actually checks never sets its channel-IRQ bit), device fully healthy throughout, no hardware touched beyond the one bounded reset+poll. **This confirms the completion race is a real, reproducible property of the first live trigger from a fresh boot, not an artifact of the earlier buggy chunking/reset logic** — with every known bug in the userspace tool now fixed, the very first attempt on a clean boot still never signals completion. The RAM-dump-as-oracle diff-check (gunzip candidate output, byte-diff against `custom/kernel_dumps/kernel_ram_dump_0x80008000.bin`) is wired up and ready but has nothing to check yet, since no attempt has produced any decrypted bytes at all.
