...
> **[Firmware Binary Analysis]** The following values were extracted from firmware binary analysis. Only decodeType 2, 3, 4, and 5 have been verified in USB captures (decodeType 3 verified via an Android Auto phone-call capture, Mar 2026). See `../02_Protocol_Reference/audio_protocol.md` for the capture-verified subset.
...
From binary analysis of the ARMadb-driver `.rodata` AGC string region. These are two **distinct** strings at **distinct** addresses: `Not all PhoneCall data Do AGC` at VMA `0x6d167` and `webrtc_Agc_Process args error!` at VMA `0x6bff2` (not the single `0x5a642` address sometimes used loosely for the whole region):
```
...
**Impact:** Even if VoiceQuality controlled sample rate selection (8kHz vs 16kHz), the setting is never applied. All **CarPlay** telephony uses 16kHz because:
1. The CallQuality→VoiceQuality translation fails
2. CarPlay independently negotiates 16kHz (wideband) with modern iPhones

(Android Auto phone calls instead use 8kHz, routed through Android Auto's Bluetooth/HFP-SCO channel — see § *Phone Call Sample Rate* below.)
...
**Firmware-supported mic sample rates:** 8000 Hz (0x1F40) or 16000 Hz (0x3E80) only — any other rate fails WebRTC AECM initialization. See § *WebRTC Audio Processing → Supported Sample Rates (Binary Analysis)* for the binary-verified detail.
...
| AGC error | `0x6bff2` | `webrtc_Agc_Process args error!` (distinct from the PhoneCall-AGC string) |
| PhoneCall AGC | `0x6d167` | `Not all PhoneCall data Do AGC: %d!!!` |
