...
| `DockPosition` | Dock/sidebar position in view area (int64 — the `viewAreaStatusBarEdge` view-area dict key is int64, not float64; live-verified 2026-05-21, `CFLDictionarySetInt64Value`) |
...
**[Firmware]** The Distributed Multimedia Service Discovery Protocol (DMSDP) framework is Huawei's distributed multimedia protocol stack (HarmonyOS/OpenHarmony). It provides the core protocol implementation for all projection protocols, handling RTP streaming, session management, crypto, and channel multiplexing.
...
| ARMadb-driver | `_SendDataToCar iSize: %d, may need send ZLP` | 0x6b823 | USB transmission (VMA; `0x5b823` cited elsewhere is the **file offset** of this same string, delta 0x10000 = load base) |
...
| Audio codec | `c2.android.vorbis.decoder` is a phone-side / AA-internal codec component, **not** the adapter's output path. The adapter's projection-audio output is PCM (AA projection codecs are AAC-LC / AAC-LC-ADTS / PCM per ARMAndroidAuto). |
...
|-----|---------|
| 0 | EC/NR function |
| 1 | Call waiting / three-way calling |
| 2 | CLI presentation |
| 3 | Voice recognition |
...
All projection protocols communicate through a MiddleMan interface server using Unix domain sockets. The table below is the **firmware-wide** interface registry: the complete set is **10 interface classes** — the 9 below plus `CBaiduCarLife_MiddleManInterface`. Per-binary enumerations are scope-dependent subsets of this set: `key_binaries.md` lists the 7 visible in that analysis, and `ARMAndroidAuto.md` reports the 8 classes embedded in that one binary (no `CAndroidMirror`).
...
| `CBaiduCarLife_MiddleManInterface` | China BaiduCarLife |
...
_Addresses in this table are **file offsets** (0x5bxxx range); add the 0x10000 load base for the VMA. (So the `_SendDataToCar` file offset 0x5b823 here is the same string as VMA 0x6b823 in the Video-Related Strings table.)_
...
**Android work modes** (from `/etc/android_work_mode`): 0 = None, 1 = AndroidAuto, 2 = CarLife, 3 = AndroidMirror, 4 = HiCar, 5 = ICCOA. See the `RiddleLinkType` enum note in MEMORY.md §Firmware Binary Analysis for the full 8-entry link-type mapping.
...
**Rotation:** size-bounded via `check_log_size: 524288 byte` — the adapter rotates the file when it reaches **512 KB**. No retention of prior rotations (the old file is truncated, not archived). On-device capture tooling should either stream via `tail -f` (see the `tail_tty.sh` session helper) or pull the file more often than the rotation cadence.
...
See `01_Firmware_Architecture/web_interface.md` for web server architecture and `05_Security_Analysis/security_findings.md` for unauthenticated api.cgi endpoint analysis.
...
```cpp
// Trust management for paired devices (~42-44KB library; key_binaries.md measures 42KB — re-measure on extracted rootfs to settle)
...
Full register map: `_evidence/MFi_research/docs/register_map.md`.
...
identity (see `05_Security_Analysis/security_findings.md` §5). There is **no FairPlay** path.
...
result=`dtmf_decode /tmp/dtmf.pcm | grep 5551234567 | wc -l`
...
```
/tmp/bluetooth/
└── [LOCAL_ADDR]/              # e.g., DE:AD:BE:EF:00:05
...
```
Example: `DE:AD:BE:EF:00:07 68851F93529776F17B9A155512568EA5 5 -1`
...
- Firmware version: 2025.02.25.1521CAY (a genuine CPC200-CCPA firmware release; sibling cluster docs — config_key_analysis/key_binaries/ARMAndroidAuto — target 2025.10.15.1127. Both are real CCPA firmwares, not a contradiction. Note: this doc shares identical byte-offsets with the 2025.10 docs, so a live re-check of which build each address set was extracted from is still warranted.)
