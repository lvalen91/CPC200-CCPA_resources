...
**Last Updated:** 2026-02-19 (Added: process architecture, MiddleMan IPC protocol, outbound send pipeline, 7 MiddleMan interfaces). The 7 listed here are the subset visible in this analysis of the firmware-wide **10**-class MiddleMan set — see `firmware_internals.md` for the canonical list; counts differ by scope.
...
- **CORRECTED (2026-03-15):** The earlier "zero config system strings" claim for ARMAndroidAuto was wrong — it **does** access the riddle config system (`GetBoxConfig`/`SetBoxConfig`/`ResetBoxConfig`, etc.). Canonical analysis and full string list: see `ARMAndroidAuto.md` §config.
...
| Binary | GetBoxConfig Caller Functions | Passes "ImprovedFluency"? | Any Code Ref? |
|--------|---------------------|--------------------------|---------------|
...
> **Note — caller counts are scope-dependent:** The counts above are **distinct calling functions**; config_key_analysis.md counts **call sites**, so per-binary totals differ (e.g. AppleCarPlay 19 call sites vs 12 functions, bluetoothDaemon 10 vs 8, ARMiPhoneIAP2 10 vs 14-get).
...
> **Analysis-target firmware:** This doc targets CCPA firmware **2025.10.15.1127**; firmware_internals.md targets **2025.02.25.1521CAY** — both are genuine CPC200-CCPA releases. The two docs cite identical byte offsets (e.g. `recv CarPlay videoTimestamp:%llu` at `0x6d139`), worth a live re-check against each labeled build.
...
- Subject: CarService
- Validity: Jul 4, 2014 - Jun 24, 2026 (per ARMAndroidAuto.md cert analysis; corrected from earlier "Apr 2026")

**Communication with ARMadb-driver:**
- IPC via raw Unix socket `/var/run/adb-driver`, using ARMAndroidAuto's own `CRiddleUnixSocketClient`
- MiddleManInterface type 5 = AndroidAuto
- **CORRECTED (per ARMAndroidAuto.md, unpacked-binary analysis 2026-03-15):** `_SendPhoneCommandToCar()`, `sendTransferData()`, `MiddleManClient_SendData()` and `libboxtrans.so` are **ARMadb-driver constructs and are NOT present in the ARMAndroidAuto binary**. ARMAndroidAuto forwards over the raw Unix socket via its aasdk/OpenAuto pipeline; the earlier "bridges via libboxtrans.so" claim here was a mis-attribution.

**Media Transport — CORRECTED (per ARMAndroidAuto.md, unpacked-binary analysis 2026-03-15):**
ARMAndroidAuto uses its own **aasdk/OpenAuto** media pipeline, **not** the Huawei DMSDP stack. The symbols `libdmsdp.so`, `DMSDPRtpSendQueueAVC/PCM/AAC`, and `DMSDPServiceOpsTriggerIFrame` are **NOT present in the ARMAndroidAuto binary strings** — they belong to ARMadb-driver / AppleCarPlay / the DMSDP libraries and were previously mis-attributed to ARMAndroidAuto here. The DMSDP RTP details below describe that separate DMSDP path (ARMadb-driver/AppleCarPlay), retained for reference:
...
The adapter firmware is built on **Huawei's DMSDP** framework, part of HarmonyOS/OpenHarmony distributed capability stack. (DMSDP acronym expansion unconfirmed against an authoritative Huawei source: this doc reads it as "Distributed Multimedia Service Discovery Protocol"; firmware_internals.md reads it as "Digital Media Streaming DisplayPort.")
...
| **libauthagent.so** | 42KB (firmware_internals lists 44KB; 2KB discrepancy unconfirmed, pending re-measurement) | Authentication — BT pairing, trust mgmt (HKS keystore), PIN auth, PBKDF2, device identity |
...
> **Note — interface count is scope-dependent:** The 7 interfaces above are those enumerated from this analysis (incl. `CAndroidMirror`; excl. `CNoAirPlay`/`CBaiduCarLife`). They are a subset of the firmware-wide **10**-class set (see `firmware_internals.md` for the canonical list); ARMAndroidAuto.md reports the **8** classes embedded in its binary (incl. `CNoAirPlay`, `CBaiduCarLife`; excl. `CAndroidMirror`). Counts differ by scope.
...
| `_SendDataToCar iSize: %d, may need send ZLP` | 0x6b823 | USB transmission (VMA; `0x5b823` seen elsewhere is the same string's **file offset**, delta 0x10000) |
...
For complete vulnerability analysis and exploitation details, see `05_Security_Analysis/security_findings.md`.
...
Stage 2: fcn.00015ee4 (session init, post-identification)
  ...
  ┌─ 0x15f78: tst r7, #1 → bit 0 → MediaPlayer/NowPlaying init (fcn.000282b8) [corrected — see note below]
...
**⚠️ CORRECTION:** DashboardInfo does NOT gate locationInformationComponent. Previous docs incorrectly stated bit 1 = Location. Actual mapping (a `DashboardInfo:5` live capture — bits 0+2 — enabled MediaPlayerEngine + RouteGuidanceEngine and produced NO vehicleStatusComponent; earlier static analysis mislabeled bit 0 as `vehicleInformation`):
- Bit 0: MediaPlayer/NowPlaying
- Bit 1: vehicleStatus
...
| 17 | **bluetoothTransportComponent** | id 0 `=1`; id 1 `IAP2-Bluetooth`; id 2 `transportSupportsiAP2Connection`(none); id 3 BT MAC `DE:AD:BE:EF:00:04` |
