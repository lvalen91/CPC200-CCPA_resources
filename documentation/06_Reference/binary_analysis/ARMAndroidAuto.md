...
**ViewArea / SafeArea (Binary-Confirmed Jun 2026 — proto descriptor):** `ARMAndroidAuto` speaks the
**aasdk openauto-v2** SDK (build path `.../AndroidAuto_Wireless/openauto-v2/../AndroidAutoSdk-v2/aasdk_proto/`).
Its embedded `VideoConfigData.proto` descriptor defines `aasdk.proto.data.VideoConfig` with exactly:
**`video_resolution`, `video_fps`, `margin_width`, `margin_height`, `density`, `additional_depth`,
`real_density`** (wrapped by the AV channel's `video_configs`). There is **no `ui_config`, no
`content_insets`, no `margins` nested message, and no `Insets` type anywhere in the binary** — this SDK
predates AAP's `ui_config`. So `ARMAndroidAuto` can emit only a **symmetric letterbox** (`margin_width` /
`margin_height` = tier − config); a content-inset SafeArea is *structurally impossible*. The AA view
rectangle is sized from the `androidAutoSizeW`/`androidAutoSizeH` BoxSettings keys
(→ `AndroidAutoWidth`/`AndroidAutoHeight`, read via `GetBoxConfig`), **not** from `HU_VIEWAREA_INFO` —
in `ARMadb-driver`, `HU_VIEWAREA_INFO`'s only xref is `fcn.00021cb0` (`ProceessCmdOpen`, the CarPlay/Open
handler). The bare main-screen `HU_SAFEAREA_INFO` is CarPlay-only (read solely by `AppleCarPlay`). A
genuine AA content-inset SafeArea requires a host speaking the AAP protocol directly, not the dongle.
See `01_Firmware_Architecture/configuration.md` § "Android Auto: margins only — no main-screen SafeArea".
...
T+5.0s   "Recv start linkParam: 2, DE:AD:BE:EF:00:07" (2=wireless)
...
T+5.2s   BoxRFCOMMService start — sends WiFi credentials via RFCOMM:
           ...
           bssid=DE:AD:BE:EF:00:02, securityMode=8, port=54321
...
- Natural IDR interval: **62-68 seconds** measured end-to-end (encoder-configured `sync-frame-interval=60s` + encoding delay)
...
The adapter's projection-audio output path is PCM/AAC-LC. (Android Auto itself can use Vorbis — `c2.android.vorbis.decoder` — but that is a phone-side/AA-internal codec, not the adapter's output codec.)
...
`BoxAudioConvertInit ctx:%p, src format:%d-%d-%d, dst format:%d-%d-%d` — nominal format-conversion routine between phone codec output and the adapter's internal audio pipeline. **Dead code:** these strings have zero code cross-references (see §28); AA negotiates audio format directly, so no runtime conversion actually occurs.
...
| channeltype | 0 |
| ip | 192.168.43.1 (adapter WiFi AP interface) |
| port | 54321 |
| ssid | carlink |
| passwd | 12345678 |
| bssid | DE:AD:BE:EF:00:02 (adapter WiFi MAC) |
...
| FERRY_BOAT | 15 | 18 | **18** | — | +2, gap at 17 (see §25) |
| FERRY_TRAIN | 16 | 19 | **0** | — | **BUG: unmapped** |
...
<< AT+CIND?
>> +CIND: 0,0,0,0,0,5,0
→ AG DE:AD:BE:EF:00:07: Connected
```
...
**ARMAndroidAuto-internal strings** (NOT in riddleBoxCfg 106-entry table):
...
The binary is NOT AA-only. It contains MiddleMan interface classes for 8 protocols — the subset embedded in *this* binary, drawn from the firmware-wide **10**-class set (canonical list in `firmware_internals.md`). The per-file counts (7/8/10) are scope-dependent:
...
**VERIFIED 2026-03-16 (r2 exhaustive xref scan):** All 4 DEAD keys are confirmed **DEAD in ARMAndroidAuto too**. They exist in the 106-entry config table (as vestigial struct entries) but none of the 26 `GetBoxConfig`/`GetBoxConfigBool` call sites in the binary pass these key names. Only 20 of 106 config keys are actually read at runtime. The DEAD classification in `config_key_analysis.md` remains correct across all 7 binaries.
