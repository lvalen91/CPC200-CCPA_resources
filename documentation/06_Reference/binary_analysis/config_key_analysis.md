...
**Firmware:** 2025.10.15.1127 (ARMadb-driver) — a genuine CPC200-CCPA release, distinct from the `2025.02` family (e.g. `2025.02.25`) analyzed in `firmware_internals.md`. Both are real CCPA firmware versions (two different releases); the coexistence is not a contradiction. Byte-offsets shared across docs remain worth a live re-check.
**Analysis date:** 2026-02-28
**Method:** Ghidra headless decompilation + r2 xref tracing across 6 binaries
**Scope:** All 106 keys (79 integer + 24 string + 3 array) — authoritative count, dumped directly from the table with addresses/strides. (ARMAndroidAuto.md's "110-entry config table" figure should be re-derived against this dump.)
...
> **Metric note:** The *Get/Set Callers* columns count GetBoxConfig/SetBoxConfig **call sites**; key_binaries' ImprovedFluency table counts **distinct calling functions**, so per-binary totals differ by scope (e.g. AppleCarPlay 19 call sites here vs 12 functions there; bluetoothDaemon 10 vs 8; ARMiPhoneIAP2 10 vs 14-get).

**CORRECTED 2026-03-16:** ARMAndroidAuto **DOES** access the riddle config system via `GetBoxConfig`/`SetBoxConfig` (reads ≥18 riddleBoxCfg keys directly, plus the AA-internal `audio_loopback`), reversing the 2026-02-28 "zero config strings" claim (the packed binary obscured them). **Canonical write-up:** `ARMAndroidAuto.md` Section 30.5 — see there for the full key list and evidence.
...
| 17 | HudGPSSwitch | 1 | 0-1 | `gps` | GPS forwarding master gate: 0=disabled, 1=enable NMEA via iAP2 (factory default 1, confirmed via `riddleBoxCfg --info`). Effective GPS still requires GNSSCapability>0 |
...
> **Zero code xrefs:** `lightType` (str `0x19cdb`), `NotCarPlayH264DecreaseMode` (`0x19b22`), and `ImprovedFluency` (`0x19de6`) each have **zero code xrefs** — each appears only as a single descriptor pointer in the `riddleConfigNameValue` config table (`~0x2b1ac`), confirming DEAD. `UDiskPassThrough` (`0x19bfc`) likewise shows zero xrefs in `riddleBoxCfg`, but is **left open** (some users report it functional) pending a check across all consumers (scripts/other binaries), so it is not asserted DEAD here.
...
- **Type:** Integer | **Default:** 0 | **Min:** 0 | **Max:** 1
- **Status:** PASS-THROUGH (exhaustive analysis 2026-02-28; server.cgi web-API serialization only, no runtime behavioral effect in any core binary). Reconciled from an earlier "CONFIRMED DEAD" label: ImprovedFluency has 2 server.cgi xrefs, so it does **not** meet the zero-xref DEAD definition and is excluded from the 12-key DEAD master list — it is PASS-THROUGH, matching ARMAndroidAuto.md's classification.
...
ImprovedFluency would have toggled a video/audio smoothing optimization, likely reducing latency at the cost of quality or vice versa. The key is present in the config table of all binaries and is exposed in the web UI via server.cgi (FUN_00014040 reads it alongside all other keys for the settings page at `http://192.168.43.1/cgi-bin/server.cgi`) — these server.cgi xrefs are why it is PASS-THROUGH rather than DEAD. However, no runtime binary consumes the value: ARMadb-driver (Android Auto -- 8617 lines, 24 functions), AppleCarPlay (3495 lines, 12 functions), ARMiPhoneIAP2 (1023 lines, 10 functions), and bluetoothDaemon (435 lines, 4 functions) all have zero xrefs. The key `ARMAndroidAuto` process (which runs the actual AA video pipeline) was not decompiled because it does not link against the riddleBoxCfg shared library at all, confirming it cannot read this key via the standard config API. The web UI lets users toggle it, the config library stores it, but nothing acts on it. Default=0 (disabled) means the non-functional toggle ships in the "off" state.
...
- **Type:** Integer | **Default:** 0 | **Min:** 0 | **Max:** 4096
- **Status:** PASS-THROUGH in core binaries — **but consumed by ARMAndroidAuto** (CORRECTED 2026-03-16: read via `GetBoxConfig` at `0x3a8a0` for resolution-tier selection; see ARMAndroidAuto.md §24). Not a pure PASS-THROUGH.
...
ARMAndroidAuto does NOT dynamically link the `riddleBoxCfg` shared library — but it **does** read the value at runtime via its own statically-compiled `GetBoxConfig` (CORRECTED 2026-03-16; the packed binary had obscured these strings, so the original "zero config strings" claim was wrong — see the CORRECTED note above and ARMAndroidAuto.md §30.5). In addition, the value also reaches ARMAndroidAuto through the boot-script pipeline:
...
**No *core* binary (ARMadb-driver, AppleCarPlay, ARMiPhoneIAP2, bluetoothDaemon) reads this config key** — their only ARMadb xrefs are the two DATA references in the BoxSettings JSON mapping table (entry [14]) and the integer config table definition. ARMAndroidAuto, however, **does** read it at runtime via `GetBoxConfig` at `0x3a8a0` (CORRECTED 2026-03-16). The value is stored, forwarded to the boot script, **and** read by ARMAndroidAuto at connection time for resolution-tier selection.
...
- **Type:** Integer | **Default:** 0 | **Min:** 0 | **Max:** 4096
- **Status:** PASS-THROUGH in core binaries — **but consumed by ARMAndroidAuto** (CORRECTED 2026-03-16: read via `GetBoxConfig` for resolution-tier selection + margin calculation; see ARMAndroidAuto.md §24). Not a pure PASS-THROUGH.
...
**No *core* binary reads this config key** — only DATA references in ARMadb's mapping table and config table definition. ARMAndroidAuto, however, **does** read it at runtime via `GetBoxConfig` for tier selection + margin calculation (CORRECTED 2026-03-16).
...
**Key ID:** 17  
**Default:** 1 (factory default, per `riddleBoxCfg --info`: `HudGPSSwitch 1 0 1` = default 1, min 0, max 1)  
**Range:** [0,1]  
...
```
WLAN silicon is variant-dependent: CCPA units shipping the Realtek RTL88x2CS support both AP and P2P modes, whereas units shipping the NXP/Marvell IW416 (`moal.ko`) are forced to P2P by the script above. The live-observed A15W unit = IW416; both chipsets are correct for their respective hardware revisions.
...
- WiFi AP interface IP is `192.168.43.1` (hardcoded in DHCP/hostapd; SSH-over-WiFi address). The `192.168.50.2` address belongs to the **separate USB NCM interface**, not the WiFi AP — they are two different interfaces, not alternatives. (Earlier "192.168.50.2 default / 192.168.43.1 if ARMHiCar" phrasing conflated the two; corrected per review canon: WiFi AP = 192.168.43.1, NCM/USB = 192.168.50.2.)
