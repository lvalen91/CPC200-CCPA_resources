...
# ON-DEMAND: in ncm_only mode WLAN+BT are NOT brought up at boot. NCM is the only
# always-on radio service. Use /script/wlan_on.sh and /script/bt_on.sh to bring the
# WiFi AP / Bluetooth stack up when needed, /script/wlan_off.sh /script/bt_off.sh to
# release. (ncm_wifi / stock still init both here.)
if [ -e /script/ncm_only ]; then
	echo "[start_main_service] ON-DEMAND: skip init_bluetooth_wifi (WLAN+BT off; use wlan_on/bt_on)"
else
