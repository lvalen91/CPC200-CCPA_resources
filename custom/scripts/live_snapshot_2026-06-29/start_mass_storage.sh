...
lsusb | grep "Bus 001.*12d1" && echo "[start_mass_storage] Huawei phone detected, exiting" && exit 0
...
if [ -d /mnt/UPAN/ICCOA ]; then
	...
	echo "[start_mass_storage] ICCOA debug: hot-swapping /usr/lib/libcarlink.so via safe copy"
	test -e /mnt/UPAN/ICCOA/libcarlink.so && (killall -9 iccoa; sync; cp /tmp/lib/libcarlink.so /usr/lib/libcarlink.so.new 2>/dev/null && mv /usr/lib/libcarlink.so.new /usr/lib/libcarlink.so)
	sync
	exit 0
fi
#For HiCar Debug
...
riddleBoxCfg -g UDiskPassThrough | grep 0 && echo "[start_mass_storage] Udisk pass through mode closed" >> /dev/console && exit 0
...
echo "[start_mass_storage] swapping in storage_common / g_android_accessory modules"
...
echo "[start_mass_storage] enabling mass_storage gadget on backing store $usb_dev_path"
...
echo "[start_mass_storage] gadget state: functions=$(cat /sys/class/android_usb_accessory/android0/functions) enable=$(cat /sys/class/android_usb_accessory/android0/enable)"
