...
if [ -e /usr/sbin/fakeiOSDevice ]; then
	echo "[start_accessory] Don't start accessory driver when use fakeiOSDevice" >> /dev/ttymxc0
	exit 0
elif [ -e /usr/sbin/fakeCarLifeDevice ]; then
	echo "[start_accessory] Don't start accessory driver when use fakeCarLifeDevice" >> /dev/ttymxc0
	exit 0
elif [ -e /usr/sbin/fakeAADevice ]; then
	echo "[start_accessory] Don't start accessory driver when use fakeAADevice" >> /dev/ttymxc0
	exit 0
elif [ -e /tmp/UDiskPassThroughMode ]; then
	echo "[start_accessory] Don't start accessory driver when UDiskPassThroughMode" >> /dev/ttymxc0
	exit 0
fi
...
# report the target mode before bouncing the external gadget into accessory (AOA)
echo "[start_accessory] external gadget -> accessory (AOA) VID=$idVendor PID=$idProduct"
...
# read back the gadget state so a failed accessory bring-up is no longer silent
echo "[start_accessory] accessory enabled state=$(cat /sys/class/android_usb_accessory/android0/state 2>/dev/null)"
